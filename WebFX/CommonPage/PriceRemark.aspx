<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PriceRemark.aspx.cs" Inherits="EyouSoft.WebFX.CommonPage.PriceRemark" %>
    ...
    <div class="alertbox-outbox02">
        <table width="99%" cellspacing="0" cellpadding="0" border="0" style="margin: 0 auto;"
            ...
            <tbody>
                ...
                <asp:Repeater runat="server" ID="rpPriceRemark">
                    <ItemTemplate>
                        <itemtemplate>
                        <tr>
        ...
        <div class="alertbox-btn">
            <a id="btnSelect" hidefocus="true" href="javascript:void(0);"><s class="xuanzhe"></s>
                <%=(String)GetGlobalResourceObject("string", "选择")%></a><a id="btnSave" href="javascript:void(0);"
                    hidefocus="true"><s class="baochun"></s><%=(String)GetGlobalResourceObject("string", "保存")%></a>
        </div>
        <br>
...
<script type="text/javascript">
    var strArr = [];
    $(function() {
        var ids = parent.$("#hfReturnValue").next("input[type='hidden']").val();
        if (ids != "") {
            var _ids = ids.split(',');
        ...
        $("#tableData").find("input[name='cbxProID']").click(function(i) {

            var self = $(this);
            var val = self.val();
            ids = parent.$("#hfReturnValue").next("input[type='hidden']").val();

            if (self.attr("checked") == true) {
                var str = parent.document.getElementById("hfReturnValue").value;
                var returnValue = $.trim($(this).parent().next().find("span[data-class='returnValue']").html()) + " </br>";
                str += returnValue;
                parent.document.getElementById("hfReturnValue").value = str;
                ...
                if (ids != "") {
                    ...
                    parent.$("#hfReturnValue").next("input[type='hidden']").val(newIds.join(','));
                }
                else {
                    parent.$("#hfReturnValue").next("input[type='hidden']").val(val);
        ...
        var SelectInformation = {
            ...
            LgType: '<%=EyouSoft.Common.Utils.GetQueryStringValue("LgType") %>',
            ...
            Save: function() {
                ...
                $.newAjax({
                    type: "post",
                    cache: false,
                    url: "/CommonPage/PriceRemark.aspx?type=save&sl=" + SelectInformation.sl + "&LgType=" + SelectInformation.LgType,
