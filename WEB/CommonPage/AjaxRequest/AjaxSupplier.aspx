<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AjaxSupplier.aspx.cs" Inherits="EyouSoft.Web.CommonPage.AjaxRequest.AjaxSupplier" %>
...
<script type="text/javascript">
    $(function() {
        var type = '<%=Request.QueryString["suppliertype"] %>';
...
<table width="100%" cellspacing="0" cellpadding="0" bgcolor="#FFFFFF" align="center"
    ...
    <tr>
        ...
        <label>
            <input name="1" class="i_bt" type="radio" value="<%#Eval("GysId") %>" data-WPrice='0.00' data-pricejs='' data-priceth='' data-show="<%#Eval("GysName")%>" data-contactname="<%#GetContactInfo(Eval("Lxrs"),"name")%>" data-fax="<%#GetContactInfo(Eval("Lxrs"),"fax")%>" data-tel="<%#GetContactInfo(Eval("Lxrs"),"tel")%>" data-mobile='<%#GetContactInfo(Eval("Lxrs"),"mobile")%>' <%#Eval("GysName")==Request.QueryString["name"]?"checked=checked":"" %> data-jiesuanfangshi="<%#Eval("JieSuanFangShi") %>"/> 
                <%if ((EyouSoft.Model.EnumType.GysStructure.GysLeiXing)EyouSoft.Common.Utils.GetInt(EyouSoft.Common.Utils.GetQueryStringValue("suppliertype")) == EyouSoft.Model.EnumType.GysStructure.GysLeiXing.酒店)%>
                <%{%>
        ...
        <%#EyouSoft.Common.UtilsCommons.GetCompanyRecommend(Eval("IsTuiJian"), Eval("IsQianDan"))%>
