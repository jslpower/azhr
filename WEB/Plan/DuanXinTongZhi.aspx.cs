...
namespace EyouSoft.Web.Plan
{
    ...
    public partial class DuanXinTongZhi : BackPage
    {
        protected string showPay = string.Empty;
        protected void Page_Load(object sender, EventArgs e)
        {
            //初始化菜单控件css
            this.PlanConfigMenu1.IndexClass = "5";
        ...
        private void Send()
        {
            ...
            string[] l = Utils.InputText(Utils.GetFormValue(this.JieShouRen.SellsIDClient)).Split(',', '，');
            System.Text.StringBuilder errBuilder = new System.Text.StringBuilder();
            string resultMess = string.Empty;
            var m = new EyouSoft.Model.ComStructure.MComUser();
            var b = new EyouSoft.BLL.ComStructure.BComUser();
            ...
            {
                m = b.GetModel(id, this.SiteUserInfo.CompanyId);
                if (m==null)continue;
                if (!Utils.IsMobile(m.ContactMobile) && !Utils.IsPhone(m.ContactMobile))//验证手机格式如果不正确则输出错误手机
                {
                    errBuilder.AppendFormat("{0}，", m.ContactMobile);
                }
                else
                {
                    EyouSoft.Model.SmsStructure.MSmsNumber Mobile = new EyouSoft.Model.SmsStructure.MSmsNumber()
                    {
                        Code = m.ContactMobile.Trim(),
            ...
            if (resultMess == "")
            {
                ...
                //发送类型
                SendMessage.SendType = EyouSoft.Model.EnumType.SmsStructure.SendType.直接发送;
                //发送时间
                ...
                //}
                SendMessage.UserFullName = this.SiteUserInfo.Name;
                #endregion
                #region 执行发送
                //执行发送，返回发送结果实体
