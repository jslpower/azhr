...
namespace EyouSoft.Web.Fin
{
    ...
    public partial class DanTuanGuaZhangDetail : BackPage
    {
        ...
        void DataInit()
        {
            ...
            if (m!=null)
            {
                this.ltlTourCode.Text = m.TourCode;
                this.ltlAreaName.Text = m.AreaName;
                this.ltlRouteName.Text = m.RouteName;
                this.ltlTourDays.Text = m.TourDays.ToString();
                this.ltlAdult.Text = m.Adults.ToString();
                this.ltlChild.Text = m.Childs.ToString();
                this.ltlLeader.Text = m.Leaders.ToString();
                this.ltlYongFangShu.Text = m.TourRoomList!=null&&m.TourRoomList.Count>0? m.TourRoomList.Count.ToString():"0";
                var g = new EyouSoft.BLL.SysStructure.BGeography().GetCountry(CurrentUserCompanyID, m.CountryId);
                this.ltlCountry.Text = g != null ? g.Name : string.Empty;
                this.ltlLDate.Text = m.LDate.ToShortDateString();
                this.ltlArriveCity.Text = m.ArriveCity;
                this.ltlArriveCityFlight.Text = m.ArriveCityFlight;
                this.ltlRDate.Text = m.RDate.ToShortDateString();
                this.ltlLeaveCity.Text = m.LeaveCity;
                this.ltlLeaveCityFlight.Text = m.LeaveCityFlight;
                this.ltlSellerName.Text = m.SellerName;
                this.ltlPlaners.Text = m.Planers;
            }
            var l = new EyouSoft.BLL.HPlanStructure.BPlan().GetList(tourId);
            if (l!=null&&l.Count>0)
            {
                this.rptList.DataSource = l;
        ...
        /// word导出
        /// </summary>
        protected void ibtnWord_Click(object sender, EventArgs e)
        {
